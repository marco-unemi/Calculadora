...
class SistemaEcuaciones:
    ...
    def resolver_sistema_de_ecuaciones(A, b):
        try:
            ...
            resultado = "\n".join(
                [f"X{''.join(SistemaEcuaciones.subindices[d] for d in str(i+1))} = {valor:.4f}" for i, valor in enumerate(x)])
            print(f"Solución:\n{resultado}")
            return resultado
    ...
    def resolver_sistema(self):
        try:
            ...
            resultado = self.resolver_sistema_de_ecuaciones(A, b)
    ...
    def renderizar_celdas(self):
        ...
        def get_subindice(num):
            return ''.join(self.subindices[d] for d in str(num))
